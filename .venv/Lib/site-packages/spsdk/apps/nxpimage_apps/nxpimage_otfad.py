...
def otfad_export_command(alignment: int, config: Config, index: int | None = None) -> None:
...
def otfad_export(alignment: int, config: Config, index: int | None = None) -> None:
    """Generate OTFAD Images from YAML/JSON configuration."""
...
def otfad_get_kek(
    ...
    if output_folder:
        ...
        click.echo(f"Result files has been stored into: {get_printable_path(output_folder)}")
