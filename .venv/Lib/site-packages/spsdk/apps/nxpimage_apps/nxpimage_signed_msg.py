...
def signed_msg_export(config: Config, assets: str | None) -> None:
...
def signed_msg_parse(family: FamilyRevision, binary: str, output: str) -> None:
    ...
    click.echo(
        f"Success. (Signed message: {get_printable_path(binary)} has been parsed "
...
def signed_msg_get_template(family: FamilyRevision, message: str | None, output: str) -> None:
...
def tlv_parse(family: FamilyRevision, binary: str, tlv_type: str, output: str) -> None:
    ...
    try:
        ...
        click.echo(
            f"Success. (TLV blob: {get_printable_path(binary)} has been parsed "
            f"and stored into {get_printable_path(output)}.)"
        )
