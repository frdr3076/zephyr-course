...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Skip generation of AHAB fuse script files alongside the image.",
)
def ahab_export_command(config: Config, no_fuse_scripts: bool) -> None:
    ...
    """
    ahab_export(config, export_fuse_scripts=not no_fuse_scripts)


def ahab_export(config: Config, export_fuse_scripts: bool = True) -> None:
    """Generate AHAB Image from YAML/JSON configuration.

    :param config: Loaded YAML/JSON configuration.
    :param export_fuse_scripts: When True (default), generate fuse script files
        alongside the image. Pass False to suppress fuse script generation.
    """
...
@click.option(
    ...
    default=False,
    help=(
        "Also fully parse NXP-signed containers (SECO/ELE/V2X). "
...
def ahab_parse_command(
    ...
    By default, NXP-signed containers (SECO/ELE/V2X) are not fully parsed.
    They are extracted as a binary file and referenced via binary_container
    so the image can be re-assembled without the NXP private keys.
    Use --parse-nxp to also parse NXP-signed containers.
    """
    ahab_parse(family, binary, dek, output, parse_nxp=parse_nxp)
...
def ahab_parse(
    ...
    :param binary: Path to binary AHAB image.
    :param dek: Data encryption keys for decryption.
    :param output: Output directory for parsed files.
    :param parse_nxp: When True, fully parse NXP-signed containers.
    ...
    write_file(
        ahab_image.get_config_yaml(parsed_folder, parse_nxp=parse_nxp),
        os.path.join(parsed_folder, "parsed_config.yaml"),
    )

    click.echo(
        f"Success. (AHAB: {get_printable_path(binary)} has been parsed "
        f"and stored into {get_printable_path(parsed_folder)}.)"
    )
...
def ahab_update_keyblob_command(
    ...
    click.echo(f"Success. (AHAB: {get_printable_path(binary)} keyblob has been updated)")
...
def ahab_re_sign_command(
    ...
    click.echo(f"Success. (AHAB: {get_printable_path(binary)} signature has been updated)")
...
def ahab_sign_command(
    ...
    click.echo(f"Signed image saved to {get_printable_path(output)}")
...
def ahab_fix_signature_block_command(family: FamilyRevision, binary: str, output: str) -> None:
    ...
    try:
        ...
        click.echo(f"Fixed image saved to {get_printable_path(output)}")
    except SPSDKError as exc:
...
def ahab_cert_block_export(config: Config, output: str, plugin: str | None = None) -> None:
    """Generate AHAB Certificate Blob from YAML/JSON configuration."""
    if not output:
...
def ahab_cert_block_parse(family: FamilyRevision, binary: str, srk_set: str, output: str) -> None:
    ...
    click.echo(
        f"Success. (AHAB Certificate Blob: {get_printable_path(binary)} "
        f"has been parsed into {get_printable_path(output)}.)"
    )
...
def ahab_keyblob_parse(config: Config, binary: str, output: str) -> None:
    ...
    try:
        ...
        click.echo(
            f"\nSuccess. (AHAB Keyblob: {get_printable_path(binary)} has been parsed "
...
def parse_dek_parameter(dek_values: tuple[str, ...]) -> dict[int | None, str]:
